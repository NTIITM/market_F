        ...
        <el-form :model="Form"  class="user-container" label-position="left" label-width="60px" size="medium">
          <el-form-item  style="float: right; width: 80%" label="用户名">
            <el-input type="text" v-model="Form.userid" autofocus ref="userid" auto-complete="off"
                      placeholder="请输入要找回密码的用户名" prefix-icon="el-icon-user-solid" spellcheck="false" :disabled="isUserid">
            </el-input>
          </el-form-item>
          <el-form-item style="float: right; width: 80%" label="邮箱号">
            <el-input type="text" v-model="Form.email" autofocus ref="email" auto-complete="off"
                      placeholder="请输入用来找回密码的邮箱" prefix-icon="el-icon-message" spellcheck="false" :disabled="!isUserid">
            </el-input>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  created() {
    ...
    if (regEmail.test(this.$route.query.userid)) {
      console.log("传来了邮箱")
      this.Form.email = this.$route.query.userid;
      this.isUserid = false;
    } else {
      // 传来的不是邮箱，那就是用户名
      console.log("传来了用户名")
  ...
  methods: {
      ...
      if (this.active === 1) {
        ...
        if (this.codeForm.code === '') {
          ...
          axios({url: '/resetPassword',
            ...
            }else if (successResponse.data.errCode == 304) {
              //验证码匹配错误返回对应信息
              //如果用户名和邮箱匹配错误，那么显示错误信息，并让按钮重新可用
              this.$notify({
                title: '失败',
                message: successResponse.data.errMsg,
                type: 'error',
                duration: 0,
                offset: 100,
              });
              this.disabled = false;
              // this.$message.error(successResponse.data.errMag);
            }
          }).catch(failResponse => {
    ...
    post(){
      ...
      axios({url: '/resetPassword',
            ...
            }else if (successResponse.data.errCode == 301) {
              //如果用户名和邮箱匹配错误，那么显示错误信息，并让按钮重新可用
              this.$notify({
                title: '失败',
                message: successResponse.data.errMsg,
...
<style scoped>
.resetPassword{
/*'../assets/imgs/bg0' + new Date().getDay() + '.jpg'*/
  /*background-image: url("../assets/resetPassword_img.jpg");*/
  /*授权码：exqrwnswhlcyeahj*/
  background-image: url("../assets/imgs/bg06.jpg");

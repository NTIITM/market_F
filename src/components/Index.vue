...
export default {
  ...
  mounted() {
    ...
    // if (this.$route.path==="/index"&&localStorage.getItem("userType")=="teacher")this.gotoPage("TimeTable")
    // else if (this.$route.path==="/index"&&localStorage.getItem("userType")=="student")this.gotoPage("StudentTimeTable")
    // 监听窗口大小 自动收缩侧边栏
  ...
  watch: {
    $route(to) {
      ...
      this.tagsArry = this.tagsArry.filter(item =>
      {
        console.log(item)
  ...
  methods: {
    ...
    userOperate(name) {
      switch (name) {
        case "1":
          ...
          else this.gotoPage("StudentChangePassword");
          break;
        case "2":
          if (localStorage.getItem("userType")=="teacher"){
            this.gotoPage("teacherinfo");
          }
          else{
            this.gotoPage("studentInfo");
          }
    ...
    closeTag(i) {
      ...
      if (this.tagsArry.length) {
        ...
        if (name != "home") {
          if (localStorage.getItem("userType")=="teacher")
          this.gotoPage("TimeTable");
          else this.gotoPage("StudentTimeTable")
        } else {
          this.reloadPage();
        }
      }
